...
while True:
    try:
        ...
        elapsed = time.ticks_diff(time.ticks_ms(), last_interrupt_time)
        if elapsed > 600:
        ...
        print(f"Desired raw PWM: {desired_pot_value :<10} | Supposed to Recieve: {measured_uart_value: <10} | Measured PWM: {measured_signal_value :<10} | Diff: {difference :<10} | PWM status: {PWM_timeout}" ) 
        time.sleep(0.5)
